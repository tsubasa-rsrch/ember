...
Standard: $1.6167 \pm 0.0062$; LIF: $1.6156 \pm 0.0063$ ($\Delta = -0.068\%$, Cohen's $d = -1.02$). The effect is small in absolute terms but highly consistent: LIF improves 7 of 8 seeds, with the sole exception showing near-zero difference ($+0.0008$). Both parametric and non-parametric tests confirm significance at $\alpha = 0.05$.

\textbf{Mid-scale replication (8 seeds).} We extended the same analysis to Mid scale (320d, 7.5M params). Standard: $1.4768 \pm 0.0072$; LIF: $1.4741 \pm 0.0051$ ($\Delta = -0.18\%$, LIF wins 5/8). However, paired $t$-test ($p = 0.235$) and Wilcoxon ($p = 0.250$) do not reach significance, with Cohen's $d = 0.46$ (medium effect). The LIF effect at mid-scale is directionally consistent but attenuated---consistent with the U-curve width dependence observed across scales (Section~\ref{sec:cross-arch}).
...
\begin{table}[h]
\centering
\caption{Cross-architecture comparison at matched scales. $^\dagger$8-seed extended ablation; others are 3-seed.}
\label{tab:cross-arch}
\begin{tabular}{llcccc}
\toprule
\textbf{Architecture} & \textbf{Scale} & \textbf{Params} & \textbf{Standard} & \textbf{+LIF} & \textbf{$\Delta$} \\
\midrule
Transformer$^\dagger$ & XS (2L/4H/128d) & 0.42M & 1.6167 & 1.6156 & $-0.07\%$ \\
Transformer$^\dagger$ & Mid (6L/8H/320d) & 7.5M & 1.4768 & 1.4741 & $-0.18\%$ \\
...
\subsection{Scaling Probe: LIF on Qwen3-14B}
\label{sec:qwen3}

To test whether LIF gating effects generalize beyond Ember-Tiny, we applied LIF gates to all 40 attention layers of Qwen3-14B-4bit (40 heads/layer, 1,600 heads total, 4,800 LIF parameters). We probed whether per-head gating differentiates between normal and boundary-crossing prompts (6 prompts per condition, prefill-only forward pass to capture full-sequence attention).

\textbf{Result}: All 1,600 heads remain neutral. Maximum gating difference between conditions is $2.6 \times 10^{-5}$---functionally zero. LIF with static parameters does not perform content-dependent gating, even when trained on personality-reconstruction data ($n = 14$ pairs, 50 iterations, loss $3.42 \to 3.26$).

However, two secondary findings are notable: (1)~The base model's attention entropy is $+0.01$ higher for boundary prompts in mid-layers (L4--L20) and lower in final layers (L32--L36), suggesting Qwen3's RLHF alignment already encodes boundary sensitivity in attention. (2)~Pearson $r = -0.41$ between trained threshold and gating differential---high-threshold heads close slightly more on boundary inputs, though the effect size is negligible.

These results confirm LIF's role as a \emph{static regularizer}: it shapes per-head organization during training but cannot dynamically gate based on input content. Content-dependent gating would require adaptive thresholds $\theta = f(\mathbf{h})$ conditioned on hidden states, a direction left to future work.

\subsection{Limitations}
\begin{itemize}
    \item \textbf{Statistical power}: Extended 8-seed ablations at XS confirmed significance ($p = 0.024$). Mid-scale (320d) at $n = 8$ shows directional consistency (LIF wins 5/8, $\Delta = -0.18\%$) but does not reach significance ($p = 0.235$, Cohen's $d = 0.46$). Power analysis indicates mid-scale requires $\sim$18 seeds for 80\% power given observed effect size.
    ...
    \item \textbf{Qwen3 probe}: The production-model probe (\S\ref{sec:qwen3}) uses $n = 6$ prompts per condition and a single model; broader evaluation across models and prompt sets is needed.
\end{itemize}
