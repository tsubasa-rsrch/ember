...
\begin{enumerate}
    ...
    \item \textbf{Architecture-dependent effectiveness}: LIF consistently improves Transformers but has zero effect on CfC networks, recapitulating the thalamus's selective gating of cortical but not cerebellar processing.
    \item \textbf{Extreme parameter efficiency}: 108 parameters achieve comparable results to Qwen's gated attention (884K parameters) at $8{,}000\times$ fewer parameters.
...
\textbf{Parameter count.} For 6-layer, 12-head Transformer: $2 \times (6 \times 9) = 108$ learnable parameters (one $\theta$ and one $k$ per gate unit). This is $8{,}000\times$ fewer than Qwen's gated attention ($884{,}736$ parameters) at the same gate position.
...
All experiments use character-level Shakespeare (1.1M characters), 6-layer 12-head Transformer (10.6M parameters), 3000 training iterations, 3 random seeds (42, 668, 1337).

\begin{table}[h]
\centering
\caption{Full-scale Transformer ablation (6L/12H/768d, 10.6M params). Mean $\pm$ std over 3 seeds.}
...
This progressive hierarchy is \emph{absent} in standard training, where all layers show similar activation patterns. The pattern mirrors cortical organization: superficial layers perform broad feature detection while deep layers perform specialized computation.
...
Inhibitory mechanisms alone \emph{degrade} performance. Only their combination with excitatory LIF gating produces the best result ($-0.79\%$). This mirrors biological findings that critical periods require both excitatory and inhibitory components working together \cite{hensch2005critical}.
...
Medium & 6L/8H/256d & 7.44M & $1.4787 \pm 0.0022$ & $1.4839 \pm 0.0037$ & $+0.35\%$ \\
Mid & 6L/8H/320d & 7.44M & $1.4739 \pm 0.0050$ & $1.4700 \pm 0.0030$ & $\mathbf{-0.26\%}$ \\
...
Transformer & Full (6L/12H/768d) & 10.6M & 1.4784 & 1.4673 & $\mathbf{-0.75\%}$ \\
\midrule
...
This architectural distinction maps directly onto brain neuroanatomy (Section~\ref{sec:neuroanatomy}).
...
\textbf{Gated Attention.} Qwen Gated Attention (NeurIPS 2025 Best Paper) introduces $Y' = Y \odot \sigma(XW_\theta)$, a query-dependent sigmoid gate. At the same gate position, our LIF gate achieves comparable results with $8{,}000\times$ fewer parameters. The key difference is biological grounding: LIF uses a threshold-based fire/smolder paradigm.
...
These structural changes suggest that LIF adds meaningful inductive bias that shapes \emph{how} the network organizes computation. This mirrors biology: the brain's hierarchical organization serves adaptability and efficiency, not necessarily peak performance on any single task.
...
Analysis of learned LIF parameters reveals that not all heads learn equivalent thresholds. In the full-scale model (6L/6H), head L0H2 emerges as an extreme outlier: threshold $0.152$ (vs.\ mean $0.012$), leak $0.661$ (vs.\ mean $0.368$), and steepness $3.25$ (vs.\ mean $2.01$). This ``gatekeeper head'' pattern---where $\sim$25\% of heads per layer develop substantially higher thresholds---is robust across seeds and scales.

A 3-seed experiment at XS scale (2L/4H) confirms that each layer consistently produces approximately one gatekeeper head, though which head assumes the role varies by random initialization. Interestingly, L1H2 shows consistent elevation across all three seeds, suggesting some positional structure.

The $\sim$25\% gatekeeper ratio is notable given the 15--20\% ratio of inhibitory interneurons in biological cortex. While we do not claim mechanistic equivalence, this convergence---emerging from gradient descent without any explicit architectural constraint---further supports the thesis that threshold-based gating leads to convergent organizational principles (see Figures~\ref{fig:head-params} and~\ref{fig:threshold-seeds} in Appendix).

\subsection{Limitations}
\begin{itemize}
    \item \textbf{Single task}: Character-level Shakespeare is controlled but narrow.
...
\caption{\textbf{v3.5 E/I balance discovery.} (Left) Mean val\_loss with error bars for 6 conditions. (Right) Seed stability showing std reduction factor. Inhibitory alone degrades; combined achieves best. Mirrors biological requirement for both E and I.}
\label{fig:ei}
\end{figure}
...
\caption{\textbf{Per-head learned LIF parameters (6L/6H).} Heatmaps of threshold, leak, and steepness for each attention head. L0H2 emerges as an extreme outlier across all three parameters (threshold $0.152$, leak $0.661$, steepness $3.25$), functioning as a ``gatekeeper head.'' Early and late layers develop high-threshold heads; middle layers pass signals through---paralleling cortical laminar organization.}
...
\caption{\textbf{Cross-seed threshold differentiation (XS, 2L/4H).} Learned threshold values across 3 seeds (42, 668, 1337). Each layer consistently produces $\sim$1 high-threshold ``gatekeeper'' head ($\sim$25\% ratio), though \emph{which} head assumes this role varies by seed. L1H2 shows consistent elevation across all seeds. The $\sim$25\% gatekeeper ratio mirrors the 15--20\% inhibitory interneuron ratio in biological cortex.}
