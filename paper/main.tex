...
\begin{itemize}
    \item \textbf{Statistical power}: All ablations use 3 random seeds. Paired $t$-tests yield no significant differences at $p < 0.05$ for any individual scale (best: XS, $p = 0.057$). Effect sizes are small ($< 1\%$ in loss), and the current seed count cannot distinguish genuine effects from noise. Additional seeds and formal power analysis are needed.
