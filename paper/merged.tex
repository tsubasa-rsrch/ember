...
The LIF effect traces a \textbf{U-shaped curve} across 5 Transformer scales in the width sweep (Appendix Table~\ref{tab:cross-scale}; Figure~\ref{fig:ucurve}). Extended-seed analysis (8 seeds at XS and Mid, 7 seeds at Small, 3 seeds at Medium/Wide) reveals a gradual crossover across three regimes:
...
\begin{enumerate}
    ...
    \item \textbf{Transitional width (192d)}: No detectable effect ($-0.023\%$, $p = 0.868$, $d = -0.071$, $N{=}7$). At 192d, the network sits at the crossover boundary: neither capacity-constrained nor redundant. This null result anchors the crossover region empirically.
    \item \textbf{All-needed width (256d)}: LIF hurts ($+0.355\%$, $p = 0.078$, $d = +1.95$, $N{=}3$). All embedding dimensions carry useful information; gating removes signal the model still needs.
...
The critical crossover lies between 256d and 320d (Figure~\ref{fig:ucurve}), where the network transitions from ``all dimensions needed'' to ``redundancy exists.'' The base-scale 8-seed result ($+0.01\%$, Table~\ref{tab:full-ablation}) suggests the loss improvement plateaus at larger scale, while organizational effects persist. Mechanistic analysis of per-head LIF parameters reveals that threshold standard deviation decreases monotonically with width ($\sigma_\theta = 0.031$ at 128d to $0.016$ at 384d; Appendix Figure~\ref{fig:lif-pca-width}), suggesting that wider models develop lower head-level diversity---consistent with redundant representations that LIF can selectively prune.
